...
$(document).ready(function () {
  $("#iphone-screens").cycle();

  if (Modernizr.mq('only all and (max-width: 480px)')) {
    var map = L.map('map').setView([37.7759, -122.4195], 13);
  } else {
    var map = L.map('map').setView([37.79160, -122.42759], 14);
  }
  ...
  $.each(locations, function (index, location) {
    var marker = L.marker([location.lat, location.lng], { icon:myIcon })
    marker.bindPopup("<strong>" + location.name + "</strong><br/><span>" + location.items.join(", ") + "</span>");
    marker.addTo(map);
  });
});
